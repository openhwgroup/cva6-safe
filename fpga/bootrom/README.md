# First Stage Bootloader for NeuroSoC host tile

This bootloader is based (copied and adapted) from the [bootloader provided on
the CVA6 repository tag 5.0.1](https://github.com/openhwgroup/cva6/tree/v5.0.1/corev_apu/fpga/src/bootrom)
adapted for the NeuroSoC design.

__WARNING:__ The current implementation of the bootrom only targets the single core lockstep configuration.
...
- update hardcored addresses in the sources (e.g. uart, spi) to match NeuroSoC design
- update dtb to match NeuroSoC design (potentially with configuration options)
