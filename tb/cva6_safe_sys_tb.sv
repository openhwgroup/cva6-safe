...
module cva6_safe_sys_tb;
  ...
  // Programs are expected to start at offset 0x1000 and the memory is splitted
  // in 16MB blocks (1 block per core, see NUM_WORDS below).
  // So, for example, in a dual core implementation with memory starting at
  // 0x80000000 the 1st core should execute a program loaded in 0x80000000
  // and the 2nd core should execute a program loaded in 0x80001000
  localparam int unsigned BEGIN_MEM_INSTR[] = {
      32'h80001000,
      (16*1024*1024)+32'h80001000
    };
  ...
  function coreid_array_t gen_coreids();
    ...
    // automatic coreid_array_t res =
    //   new[cva6_apu_config_pkg::CVA6APUConfigNumCores];
    automatic coreid_array_t res = new[num_cores];
    // for (int unsigned coreid = 0;
    //     coreid < cva6_apu_config_pkg::CVA6APUConfigNumCores;
    //     coreid++) begin
  ...
  begin
    ...
    binary_slot = split_using_delimiter_fn(input_offset, binary_mem, ",",
      next_offset);
    bs_file = split_using_delimiter_fn(0, binary_slot, ":", bs_next_offset);
    bs_addr_str = split_using_delimiter_fn(bs_next_offset+1,
      binary_slot, ":", bs_next_offset);
    bs_addr = bs_addr_str.atohex();
    $display("Loading '%s' to memory address 0x%x", bs_file, bs_addr);
    $readmemh(bs_file,
      dut.i_sram.gen_cut[0].i_tc_sram_wrapper.i_tc_sram.init_val,
      (bs_addr - 'h80000000)/32);

    while (next_offset > input_offset) begin
      input_offset = next_offset + 1;
      binary_slot = split_using_delimiter_fn(input_offset, binary_mem, ",",
        next_offset);
      if (next_offset == input_offset) continue;
      bs_file = split_using_delimiter_fn(0, binary_slot, ":", bs_next_offset);
      bs_addr_str = split_using_delimiter_fn(bs_next_offset+1,
        binary_slot, ":", bs_next_offset);
      bs_addr = bs_addr_str.atohex();
