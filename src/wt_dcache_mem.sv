...
//
// Modified by: Abdou Lahat NDIAYE, Thales cortAIx Labs, 2025
// Changes: SRAM arrays externalized via dcache_req/dcache_resp ports,
//          ECC error detection added on cache hit path.
//
...
#(
    ...
    // sram extern ports
    output dcls_cva6_cache_pkg::dcache_ext_req_t dcache_req,
    input  dcls_cva6_cache_pkg::dcache_ext_resp_t dcache_resp
);

  localparam DCACHE_NUM_BANKS = dcls_cva6_cache_pkg::DCACHE_NUM_BANKS;
  ...
  logic [CVA6Cfg.DCACHE_SET_ASSOC-1:0] [CVA6Cfg.DCACHE_TAG_WIDTH:0] vld_tag_rdata;

  ///////////////////////////////////////////////////////
  // connexion to sram extern arrays
  ///////////////////////////////////////////////////////

  assign dcache_req.bank_req = bank_req;
  assign dcache_req.bank_we = bank_we;
  assign dcache_req.bank_idx = bank_idx;
  assign dcache_req.bank_wuser = bank_wuser;
  assign dcache_req.bank_wdata = bank_wdata;
  assign dcache_req.bank_be = bank_be;
  assign bank_ruser = dcache_resp.bank_ruser;
  assign bank_rdata = dcache_resp.bank_rdata;

  assign dcache_req.tag_req = vld_req;
  assign dcache_req.tag_we = vld_we;
  assign dcache_req.tag_idx = vld_addr;
  assign dcache_req.tag_wdata =  wr_cl_tag_i;
  assign dcache_req.tag_wdata_vld = vld_wdata;
  assign tag_rdata     = dcache_resp.tag_rdata;
  assign rd_vld_bits_o = dcache_resp.tag_rdata_vld;
  ...
  for (genvar i = 0; i < CVA6Cfg.DCACHE_SET_ASSOC; i++) begin : gen_tag_cmpsel
    ...
    assign rd_hit_oh_o[i] = (rd_tag == tag_rdata[i]) & rd_vld_bits_o[i] & cmp_en_q & ~dcache_resp.ecc_error;
